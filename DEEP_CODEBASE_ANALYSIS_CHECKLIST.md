...
- [x] **W-01** Cancel Booking button has no `onClick` handler — button renders but does nothing
  - File: `apps/web/src/app/bookings/[id]/page.tsx:338`
  - Fix: Add onClick with confirm dialog + bookingApi.cancel() + toast
- [x] **W-02** "Message Vendor" links to `/chat?vendorId=...` but `/chat` page doesn't exist (404)
...
- [x] **A-01** Admin App.tsx imports `Users` page that doesn't exist — crashes at route
  - File: `apps/admin/src/App.tsx:7` → `apps/admin/src/pages/Users.tsx` missing
  - Fix: Create Users page scaffold
- [x] **A-02** Admin App.tsx imports `Payments` page that doesn't exist — crashes at route
  - File: `apps/admin/src/App.tsx:8` → `apps/admin/src/pages/Payments.tsx` missing
  - Fix: Create Payments page scaffold

### Flutter Mobile App

- [x] **M-01** Incorrect Riverpod state update — `state.whenData()` used as statement not assignment
  - File: `apps/mobile/lib/providers/booking_provider.dart:31`
  - Fix: Use `state = AsyncValue.data([booking, ...(state.value ?? [])])`
- [x] **M-02** Same incorrect state pattern in notifications markRead/markAllRead
...
- [x] **D-01** Docker compose services missing shared package volume mounts
...
- [x] **D-02** Docker compose missing 4 services: review, search, chat, media
